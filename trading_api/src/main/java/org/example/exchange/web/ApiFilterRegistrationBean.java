package org.example.exchange.web;
...
import org.example.exchange.bean.AuthToken;
import org.example.exchange.ctx.UserContext;
import org.example.exchange.model.ui.UserProfileEntity;
import org.example.exchange.support.AbstractFilter;
import org.example.exchange.user.UserService;
...
public class ApiFilterRegistrationBean extends FilterRegistrationBean<Filter> {
    ...
    class ApiFilter extends AbstractFilter {
        ...
        Long parseUserFromApiKey(String base64apiKey, String base64apiSignature, HttpServletRequest request) {
            // TODO: 验证API-Key, API-Secret并返回userId
            String apiKey = new String(Base64.getUrlDecoder().decode(base64apiKey));
            String apiSignature = new String(Base64.getUrlDecoder().decode(base64apiSignature));
            ApiKeyAuthEntity aka = userService.getApiKeyAuthByApiKey(apiKey);
            String apiSecret = aka.apiSecret;
            long expiresAt=aka.expiresAt;
            long userId = aka.userId;
            String toSignature = apiKey +
                    ":" + userId +
                    ":" + apiSecret +
                    ":" + expiresAt +
                    ":" + request.getHeader("API-Timestamp") +
                    ":" + request.getMethod();
            ApiKey ak = ApiKey.getFromSignature(apiSignature,toSignature,hmacKey);
            if(ak.isExpired()){
                return null;
            }
            if (logger.isDebugEnabled()) {
                logger.debug("parse from ApiKey: {}", ak.userId());
            }
            return ak.userId();
        }
