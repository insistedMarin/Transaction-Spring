...
import org.example.exchange.assets.Asset;
import org.example.exchange.assets.Transfer;
import org.example.exchange.bean.OrderBookBean;
import org.example.exchange.enums.AssetEnum;
...
import org.example.exchange.match.MatchDetailRecord;
import org.example.exchange.match.MatchResult;
import org.example.exchange.message.ApiResultMessage;
import org.example.exchange.message.NotificationMessage;
import org.example.exchange.message.event.AbstractEvent;
import org.example.exchange.message.event.OrderCancelEvent;
import org.example.exchange.message.event.OrderRequestEvent;
import org.example.exchange.message.event.TransferEvent;
import org.example.exchange.messaging.MessageConsumer;
...
import org.example.exchange.redis.RedisCache;
import org.example.exchange.redis.RedisService;
import org.example.exchange.store.StoreService;
import org.example.exchange.support.LoggerSupport;

import org.example.exchange.util.IpUtil;
import org.example.exchange.util.JsonUtil;
...
public class TradingEngineService extends LoggerSupport {
    ...
    private void runTickThread() {
        ...
        for (;;) {
            ...
            for (;;) {
                // 从tickQueue尝试获取消息，超过1000条 或者没有消息中止
    ...
    private void runOrderBookThread() {
        ...
        for (;;) {
            // 获取OrderBookBean的引用，确保后续操作针对局部变量而非成员变量: 避免在迭代过程中 latestOrderBook 发生变化。
